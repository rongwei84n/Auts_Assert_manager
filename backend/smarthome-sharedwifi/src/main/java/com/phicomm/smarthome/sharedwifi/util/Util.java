...
import com.phicomm.smarthome.sharedwifi.middleware.PhicommHttpClient;
...
public class Util {
     ...
     * @return
     */
    public static PhicommAccountDetailModel getPhicommAccountByToken(Map<String, String> headerParas) {
        PhicommAccountDetailModel parsedObj = null;
        
        try {
            String accountJson = PhicommHttpClient.httpsGet(ThirdParty.PHICOMM_CLOUD_URL_ACCOUNT_DETAIL, 
                    Const.DEFAULT_CHARSET,
                    headerParas);

            if (StringUtil.isNotEmpty(accountJson)) {
                ObjectMapper objectMapper = new ObjectMapper();
                parsedObj = objectMapper.readValue(accountJson, PhicommAccountDetailModel.class);
                if (parsedObj == null || parsedObj.getData() == null) {
                    return null;
                }
                
                if (!StringUtil.equals(parsedObj.getError(), "0")) {// 根据token获取账户信息失败
                    logger.d("Get phicomm account error " + parsedObj.getError());
                    return null;
                }
            }
        } catch (Exception e) {
